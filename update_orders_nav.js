...
files.forEach(({file, active}) => {
    ...
    // Regex to match the entire orders-nav block
    ...
    const newNav = `<nav class="orders-nav" style="border-bottom: 1px solid var(--border); margin-bottom: 20px; background: var(--bg);">
        <div class="nav-content" style="display: flex; justify-content: center; padding: 0 20px;">
            <div class="nav-links" style="display: flex; gap: 2rem; padding-top: 10px;">
                <a href="/orders" class="nav-link" data-i18n="order_tab_fetching" style="${active === 'fetching' ? styleActive : styleInactive}">Fetching</a>
